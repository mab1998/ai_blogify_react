...
const LandingPage = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Dynamically add the CSS stylesheets
    const stylesheets = [
      { id: 'tailwind-css', href: '../assets/css/tailwind.css' },
      { id: 'swiper-css', href: '../assets/css/swiper-bundle.min.css' },
      { id: 'animate-css', href: '../assets/css/animate.css' }
    ];
    ...
    // Cleanup function to remove the stylesheets on component unmount
    return () => {
    ...
    {currentUser ? (
      ...
      {currentUser && (
        <button
          ...
          className="dashboardBtn px-[22px] py-2 text-base font-medium text-white hover:opacity-70"
        >
          Go to Dashboard
        </button>
      )}
